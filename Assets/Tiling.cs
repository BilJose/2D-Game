...
public class Tiling : MonoBehaviour
{
    ...
    void Update()
    {
        if (hasLeftBuddy == false || hasRightBuddy == false)
        {
            ...
            if(cam.transform.position.x >= edgeVisiblePositionLeft-offsetX && hasLeftBuddy == false)
            {
                MakeNewBuddy(1);
                hasLeftBuddy = true;
            }else if(cam.transform.position.x <= edgeVisiblePositionRight+offsetX && hasRightBuddy == false)
            {
                MakeNewBuddy(-1);
                hasRightBuddy= true;
            }
